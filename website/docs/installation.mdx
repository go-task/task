...
FROM ubuntu:22.04
# ...
COPY --from=ghcr.io/go-task/task:v3.40.1 /usr/local/bin/task /usr/local/bin/task
COPY --from=ghcr.io/go-task/task:v3.40.1 /etc/completion/bash/task.bash /etc/bash_completion.d/task
# ...
```

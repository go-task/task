...
func (e *Executor) watchTasks(calls ...taskfile.Call) error {
	...
	log.Errf(log.Green, "task: Started watching for tasks: %s\n", strings.Join(tasks, ", "))
	...
	log.VerboseOutf(log.Green, "task: Watching for changes every %v", watchInterval)
	...
	go func() {
		for {
			select {
			case event := <-w.Event:
				log.VerboseErrf(log.Magenta, "task: received watch event: %v\n", event)
	...
	registerTaskFiles = func(c taskfile.Call) error {
		...
		for _, s := range task.Sources {
			...
			for _, f := range files {
				...
				log.VerboseOutf(log.Green, "task: watching new file: %v", absFile)
			}
		}
		return nil
	}

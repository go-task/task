...
// TODO: Merge these with the taskfile equivalents
...
// given path with any of the default Task config files names. If any of these
...
func Exists(path string) (string, error) {
	...
	for _, taskfile := range defaultTaskRCs {
	...
	return "", errors.TaskRCNotFoundError{URI: path, Walk: false}
}
...
func ExistsWalk(path string) (string, error) {
	...
	for {
		...
		if path == parentPath || (parentOwner != owner) {
			return "", errors.TaskRCNotFoundError{URI: origPath, Walk: false}
		}

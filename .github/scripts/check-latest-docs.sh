...
# release, `website/src/latest/docs` documents the released version served on
# taskfile.dev. Fixing the published docs is allowed, but the fix has to land in
# both places -- cmd/release overwrites the latest copy at every release, so a
# fix applied only there is silently lost.
#
...
latest_dir="website/src/latest/docs"
...
if [[ ${#missing[@]} -eq 0 ]]; then
  echo "Latest docs are consistent with the next docs."
  exit 0
fi

echo "The following files were changed in $latest_dir without the matching" >&2
...
echo >&2
echo "cmd/release overwrites $latest_dir at every release, so a fix applied" >&2

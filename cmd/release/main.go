...
func changelog(version *semver.Version) error {
	...
	if err := os.WriteFile(changelogSource, []byte(changelog), 0o600); err != nil { //nolint:gosec
		return err
	}
	...
	return os.WriteFile(changelogTarget, []byte(changelogWithFrontmatter), 0o600) //nolint:gosec
}
